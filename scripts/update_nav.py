...
OFFICIAL_MONTHLY_RETURNS = {
    "2025-02": 0.55, "2025-03": -0.58, "2025-04": 0.69,
    "2025-05": 0.55, "2025-06": 0.46, "2025-07": 0.64,
    "2025-08": 0.07, "2025-09": 0.28, "2025-10": 0.46,
    "2025-11": -0.11, "2025-12": 0.15,
    "2026-01": 0.59, "2026-02": 0.21, "2026-03": -1.66,
    "2026-04": 0.78, "2026-05": 0.58, "2026-06": 0.29,
}
OFFICIAL_ANCHOR_NAV = 101.48  # 30.06.2026
...
def official_monthly_baseline():
    """Monatliche NAV-Stützpunkte aus Santander-Factsheet-Renditen.
    Die Werte werden am veröffentlichten NAV 101,48 vom 30.06.2026 verankert.
    """
    nav={"2026-06":OFFICIAL_ANCHOR_NAV}
    curr=OFFICIAL_ANCHOR_NAV
    for ym in reversed(list(OFFICIAL_MONTHLY_RETURNS.keys())):
        curr=curr/(1+OFFICIAL_MONTHLY_RETURNS[ym]/100.0)
        nav[month_before(ym)]=curr
    return {last_weekday(ym):round(v,4) for ym,v in nav.items()}
...
def main():
    old=load_existing()
    merged=official_monthly_baseline()
    # Bereits gespeicherte/exakte Werte überschreiben die rekonstruierten Monatswerte.
    merged.update({x["date"]:float(x["close"]) for x in old.get("values",[]) if x.get("date") and x.get("close") is not None})
    sources=["Santander Factsheet (Monatswerte)"]
    ...
    cutoff=(datetime.now().date()-timedelta(days=650)).isoformat()
